...
export async function setupDatabase() {
  ...
  try {
    ...
    // Certificate Templates Table (stores visual templates for certificates)
    await connection.query(`
      CREATE TABLE IF NOT EXISTS \`hw-certificate-templates\` (
        id INT AUTO_INCREMENT PRIMARY KEY,
        name VARCHAR(255) NOT NULL,
        type ENUM('PARTICIPANT', 'WINNER', 'OTHER') DEFAULT 'OTHER',
    ...
    // Link certificates to a template when available
    ...
    // Committees Table
    await connection.query(`
      CREATE TABLE IF NOT EXISTS \`hw-committees\` (
        id INT AUTO_INCREMENT PRIMARY KEY,
        name VARCHAR(255) NOT NULL,
        description TEXT,
        display_order INT DEFAULT 0,
    ...
    // Committee Members Table (Organizing Committee)
    await connection.query(`
      CREATE TABLE IF NOT EXISTS \`hw-committee-members\` (
        id INT AUTO_INCREMENT PRIMARY KEY,
        name VARCHAR(255) NOT NULL,
        ...
        FOREIGN KEY (committee_id) REFERENCES \`hw-committees\`(id) ON DELETE SET NULL
      )
    `);

    // Add new portfolio-related columns if they don't exist (for existing databases)
    await addColumnIfNotExists(connection, 'hw-committee-members', 'headline VARCHAR(255)');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'location VARCHAR(255)');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'short_bio TEXT');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'interests TEXT');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'career_objective TEXT');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'skills TEXT');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'tech_skills TEXT');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'soft_skills TEXT');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'tools TEXT');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'achievements TEXT');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'experience TEXT');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'projects TEXT');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'projects_detail TEXT');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'certifications TEXT');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'education TEXT');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'resume_url VARCHAR(500)');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'edit_password VARCHAR(255)');
    await addColumnIfNotExists(connection, 'hw-committee-members', 'committee_id INT');

    // Accommodation Queries Table
    await connection.query(`
    ...
    // Add payment columns if table already exists
    await addColumnIfNotExists(connection, 'hw-accommodation-queries', 'razorpay_order_id VARCHAR(255)');
    await addColumnIfNotExists(connection, 'hw-accommodation-queries', 'razorpay_payment_id VARCHAR(255)');
    await addColumnIfNotExists(connection, 'hw-accommodation-queries', 'razorpay_signature VARCHAR(255)');
    await addColumnIfNotExists(connection, 'hw-accommodation-queries', 'payment_status ENUM(\'PENDING\', \'SUCCESS\', \'FAILED\') DEFAULT \'PENDING\'');
    await addColumnIfNotExists(connection, 'hw-accommodation-queries', 'amount DECIMAL(10, 2)');
    await addColumnIfNotExists(connection, 'hw-accommodation-queries', 'invoice_url VARCHAR(500)');

    // Gallery Media Table
    await connection.query(`
      CREATE TABLE IF NOT EXISTS \`hw-gallery-media\` (
        id INT AUTO_INCREMENT PRIMARY KEY,
        cloudinary_public_id VARCHAR(255) NOT NULL UNIQUE,
        cloudinary_url TEXT NOT NULL,
        media_type ENUM('image', 'video') NOT NULL,
        caption TEXT,
        category ENUM('Day 1', 'Day 2', 'Day 3', 'Tech Events', 'Hackathon', 'Winners', 'Fun Moments') DEFAULT 'Hackathon',
        team_name VARCHAR(255),
