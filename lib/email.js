...
export async function sendCAApprovalEmail(email, name, caCode) {
  try {
    ...
    if (error) {
      console.error('Error sending CA approval email:', error);
      return { success: false, error: error.message };
    }

    console.log('CA approval email sent successfully:', data);
    return { success: true, data };
  } catch (error) {
    console.error('Exception sending CA approval email:', error);
    return { success: false, error: error.message };
  }
}
