...
class RateLimiter {
  constructor(requestsPerSecond = 2) {
    ...
    this.minInterval = 1000 / requestsPerSecond; // 500ms between requests
...
const emailRateLimiter = new RateLimiter(2); // 2 req/s
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (isRateLimit && attempt < maxRetries - 1) {
        ...
        console.log(`[EMAIL] Rate limit hit, retrying in ${delay}ms (attempt ${attempt + 1}/${maxRetries})`);
...
export async function sendCAApprovalEmail(email, name, caCode) {
  try {
    ...
    const result = await emailRateLimiter.execute(async () => {
    ...
    if (result.error) {
      console.error('[EMAIL] Error sending CA approval email:', result.error);
...
export async function sendTaskAssignmentEmail(email, name, task) {
  try {
    ...
    if (result.error) {
      console.error('[EMAIL] Error sending task assignment email:', result.error);
      console.error('[EMAIL] Error details:', JSON.stringify(result.error, null, 2));
      return { success: false, error: result.error.message || 'Unknown error' };
    }
...
/**
 * Send emails sequentially with rate limiting (for batch operations)
 */
export async function sendEmailsSequentially(emailFunctions) {
  const results = [];
  for (const emailFn of emailFunctions) {
    try {
