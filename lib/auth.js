...
export async function createCASession(caData) {
  const token = await new SignJWT({ ...caData, role: 'ca' })
    .setProtectedHeader({ alg: 'HS256' })
    .setIssuedAt()
    .setExpirationTime('24h')
    .sign(JWT_SECRET);
...
export async function verifyCASession(token) {
  try {
    const { payload } = await jwtVerify(token, JWT_SECRET);
    return payload.role === 'ca' ? payload : null;
  } catch (err) {
    return null;
  }
}